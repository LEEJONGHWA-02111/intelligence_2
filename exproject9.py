...
class ImprovedObjectFollower(Node):
    def __init__(self):
        ...
        # Publisher: 처리된 이미지 (/tracked_image) – 디버깅 및 PC 모니터링용
        self.image_pub = self.create_publisher(CompressedImage, '/tracked_image', 10)

        self.bridge = CvBridge()

        # YOLO 모델 로드 – 실제 엔진 파일 경로와 task="detect"를 확인하세요.
        self.model = YOLO('/home/amr1_best.engine', task="detect")

        # ---------------- 파라미터 (실험으로 조정) ----------------
        self.alpha = 0.4                     # smoothing factor
        self.desired_min_height = 240        # bounding box 높이가 이 값 미만이면 객체가 멀다고 판단 → 전진
        self.desired_max_height = 270        # bounding box 높이가 이 값 초과하면 객체가 너무 가까워 → 후진
        ...
        # 로컬 컴퓨터 환경에서 cv2 창이 잘 뜨도록 창을 미리 생성
        cv2.namedWindow("Tracked Image", cv2.WINDOW_NORMAL)
        cv2.startWindowThread()

        # 프레임 처리 스레드 시작 (별도의 스레드에서 YOLO 추론)
    ...
    def process_frames(self):
        ...
        while rclpy.ok():
            ...
            # 타겟 객체를 찾지 못하면 target_id 초기화 후 반복
    ...
    def publish_twist(self):
        """타이머 콜백: 최신 정보를 이용해 Twist 명령 계산 및 발행"""
        with self.lock:
    ...
    def publish_tracked_image(self):
        """타이머 콜백: 처리된 이미지를 /tracked_image 토픽으로 발행하고, 로컬 창에 표시"""
        with self.lock:
