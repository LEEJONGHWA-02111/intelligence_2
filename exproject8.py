...
class ImprovedObjectFollower(Node):
    def __init__(self):
        ...
        # Publisher: Twist 명령 발행 (실제 로봇에서는 /cmd_vel)
        self.cmd_vel_pub = self.create_publisher(Twist, '/cmd_vel', 10)
        # Publisher: 처리된 이미지 (디버깅 및 PC 모니터링용)
        ...
        self.desired_min_height = 240        # bounding box 높이가 이 값 미만이면 전진 (객체 멀리)
        self.desired_max_height = 270        # bounding box 높이가 이 값 초과하면 후진 (객체 너무 가까움)
        self.center_threshold = 20           # 이미지 중심 오차 허용 범위 (픽셀 단위)
        self.angular_gain = 0.005            # 각속도 제어 계수
        self.linear_speed = 0.1              # 전진/후진 속도
        # -----------------------------------------------------------
        ...
        # 자동으로 사용할 카메라 인덱스 선택 (get_camera_index() 사용)
    ...
    def process_frames(self):
        """웹캠 프레임을 읽어 YOLO 추론을 수행하고, 객체 정보를 업데이트하는 스레드"""
        smoothed_box = None
        while rclpy.ok():
            ...
            # 만약 타겟 객체를 찾지 못하면 target_id 초기화
            ...
            # 디버깅용: YOLO에서 그린 annotated image (없으면 원본 프레임)
    ...
    def publish_tracked_image(self):
        """타이머 콜백: 처리된 이미지를 /tracked_image 토픽으로 발행하고 로컬 창에도 표시"""
        with self.lock:
